...
function defaultPrompt(scene: string) {
  const byScene: Record<string, string> = {
    ...
    nonwoven_bag:
      "将地毯产品无损抠图并用于商用品牌陈列联动视觉（与无纺布袋陈列同场景），地毯本体细节与logo完全保持原样，边缘干净无阴影。门店入口商业氛围，构图强调品牌统一识别，2K超清超写实。",
    tumbler:
      "将地毯产品完整抠图后合成到门店迎宾区与保温杯陈列联动场景，地毯logo与材质细节保持不变，不允许模糊或重绘。白天自然光，干净地面，大面积展示地毯，2K超清超写实电商广告风。",
    packaging:
      "将地毯产品无损抠图后合成到门店收银/包装展示区域，地毯本体纹理与logo一字不改，边缘干净锐利。画面突出品牌商业价值与礼品包装营销氛围，2K超清、超写实。",
    apparel:
      "将地毯产品完整抠图并合成在服饰门店双开门入口，保持地毯材质、纹理、logo完全一致；地毯面积要大、比例美观，不得细长条。自然光斜射、店内明亮高档、构图适合淘宝直通车，2K超清超写实。",
  };
  return byScene[scene] ?? byScene.door_mat;
}
